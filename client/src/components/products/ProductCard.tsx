...
export const ProductCard: React.FC<ProductCardProps> = ({ product, viewMode = "grid" }) => {
  ...
  return (
    <Link to={`/products/${product.id}`} className="block h-full group">
      <motion.div
        whileHover={{ y: -4 }}
        className={`bg-white rounded-xl border border-gray-300 shadow-sm hover:shadow-lg hover:border-blue-400 transition-all duration-300 overflow-hidden flex flex-col h-full ${isListView ? "flex-row items-center min-h-0" : ""
          }`}
      >
        {/* Image/Icon Section */}
        <div
        ...
        >
          ...
          <div className="absolute top-2 right-2">
            <span
              className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold border ${product.isActive
        ...
        <div className={`p-3 flex flex-col flex-1 ${isListView ? "py-3 sm:py-4" : ""}`}>
          <div className={`${isListView ? "flex-1" : "flex-1 space-y-1.5"}`}>
            ...
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[10px] uppercase tracking-wider font-extrabold text-blue-700 bg-blue-50 border border-blue-200 px-2 py-0.5 rounded-md">
