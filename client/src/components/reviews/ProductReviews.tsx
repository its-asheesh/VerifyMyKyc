...
export const ProductReviews: React.FC<ProductReviewsProps> = ({ productId, showList = true, showForm = true, showStats = true }) => {
  const queryClient = useQueryClient()

  const { data, isLoading, error } = useQuery({
    queryKey: ["product-reviews", productId],
    queryFn: () => reviewApi.getProductReviews(productId, { page: 1, limit: 20 }),
    staleTime: 30_000,
  })
      ...
      {!hasToken ? (
        ...
        >
          ...
          {createMutation.isSuccess && (
            <div className="text-xs text-green-700">Thank you! Your review has been posted.</div>
          )}
        </form>
      )}
    </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {data?.items?.map((r: Review) => (
              <div key={r._id} className="border rounded-lg p-4">
                ...
                <div className="mt-1 text-gray-700 text-sm whitespace-pre-wrap">{r.comment}</div>
