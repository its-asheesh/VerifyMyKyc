...
export const createRazorpayOrder = async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.error('Razorpay order creation error:', { error: error.message, stack: error.stack });
    return res.status(500).json({
      success: false,
...
export const updateOrderAfterPayment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.error('Error updating order:', { error: error.message, orderId, paymentId });
