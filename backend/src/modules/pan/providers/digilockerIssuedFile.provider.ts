...
import {
  DigilockerIssuedFileRequest,
  DigilockerIssuedFileResponse,
...
export async function digilockerIssuedFileProvider(
  ...
  try {
    ...
    const response = await digilockerApiClient.post(
    ...
    logger.error('Digilocker Issued File Error:', {
    ...
    const { message, statusCode } = createStandardErrorMapper(
      'Digilocker Issued File fetch failed',
    )(error);
