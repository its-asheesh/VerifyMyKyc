...
function getServiceAccount() {
  ...
  const defaultRel = path.resolve(
    process.cwd(),
    'src/modules/auth/verifymykyc-5f02e-firebase-adminsdk-fbsvc-41079032c4.json',
  );
  if (fs.existsSync(defaultRel)) {
    return JSON.parse(fs.readFileSync(defaultRel, 'utf8'));
  }
  throw new Error(
    'Service account JSON not found. Set FIREBASE_SERVICE_ACCOUNT or FIREBASE_SERVICE_ACCOUNT_PATH.',
  );
}
...
export async function gmailSend({
  ...
  if (!process.env.GMAIL_DELEGATED_USER) {
    throw new Error(
      'GMAIL_DELEGATED_USER is required for Gmail API send (a Workspace mailbox to send as).',
    );
  }
  const auth = getJWT();
  ...
  const from =
    process.env.FROM_EMAIL || process.env.GMAIL_DELEGATED_USER || 'no-reply@your-domain.com';
